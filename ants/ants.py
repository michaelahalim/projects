...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        place = self.place
        while place != hive:
            if place.bees and (self.min_range <= position <= self.max_range):
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        # Creates a copy to avoid skipping elemetns
        bees_copy_list = list(place.bees)
        if place.ant == None:
            for bees in bees_copy_list:
...
# The WallAnt class
class WallAnt(Ant):
    """WallAnt is an Ant that does nothing but have a large armor value."""
    name = 'Wall'
    implemented = True
    food_cost = 4
...
class BodyguardAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 9
        if self.contained_ant == None and other.is_container != True:
            return True
        return False
        # END Problem 9
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        ants_behind_queen = self.place.exit
        while ants_behind_queen:
            if ants_behind_queen.ant != None:
                if ants_behind_queen.ant.is_doubled == False:
                    ants_behind_queen.ant.damage *= 2
                    ants_behind_queen.ant.is_doubled = True
                if ants_behind_queen.ant.is_container and ants_behind_queen.ant.contained_ant != None and ants_behind_queen.ant.contained_ant.is_doubled == False:
                    ants_behind_queen.ant.contained_ant.damage *= 2
                    ants_behind_queen.ant.contained_ant.is_doubled = True
            ants_behind_queen = ants_behind_queen.exit
...
def apply_effect(effect, bee, duration):
    ...
    after_effect = effect(original_effect, bee)
    def effect_action(colony):
        nonlocal duration
        if duration > 0:
            after_effect(colony)
        else:
            if effect == make_scare and duration == 0:
                bee.movement = -bee.movement
